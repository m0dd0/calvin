...
def get_original_initial_evaluation_conditions() -> list[
    tuple[dict[str, int | str], list[str], np.ndarray, np.ndarray]
]:
    """Returns the initial consitions that are used for evaluation.

    This function returns the same values as one would obtain by running the get_sequences function and then 
    taking the initial state of each sequence and converting it to the corresponding robot_obs and scene_obs 
    using the get_env_state_for_initial_condition function. The reason for having this function is that it 
    that the pyhash function that is used for generating those values is heavily outdated and the more modern 
    replacements of pyhash that might be used in this codebase in future are not guaranteed to produce the 
    same values, which would make it impossible to reproduce the exact same evaluation sequences for benchmarking.
    By having this function, we can ensure that we can always get the same initial conditions for evaluation, 
    ...
    initial_conditions = [
        ...
        for initial_state, task_sequence, robot_obs, scene_obs in initial_conditions
    ]
